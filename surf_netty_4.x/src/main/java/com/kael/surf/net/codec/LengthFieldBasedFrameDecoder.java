...
public class LengthFieldBasedFrameDecoder extends ByteToMessageDecoder {
    ...
    private static final byte[] POLICY_STR_BYTES = ("<policy-file-request/>" + '\0').getBytes();

    private static final byte[] CRLF_BYTES = new String("\r\n\r\n").getBytes();

    private static final byte[] GET_BYTES = new String("GET").getBytes();
    
    private static final AttributeKey<Integer> countKey = AttributeKey.valueOf("flag");
    ...
    public LengthFieldBasedFrameDecoder(
            ...
            int lengthAdjustment, int initialBytesToStrip, boolean failFast) {
    	this(byteOrder, maxFrameLength, lengthFieldOffset, lengthFieldLength, lengthAdjustment, initialBytesToStrip, failFast, false, false);
    }
     ...
     * @param failFast
     * @param skipPolicy
     * @param skipTencentGTWHead
     */
    public LengthFieldBasedFrameDecoder(
            ByteOrder byteOrder, int maxFrameLength, int lengthFieldOffset, int lengthFieldLength,
            int lengthAdjustment, int initialBytesToStrip, boolean failFast,boolean skipPolicy,boolean skipTencentGTWHead) {
        if (byteOrder == null) {
    ...
    private boolean isEqual(final byte[] src,final byte[] dst){
    ...
    private boolean isBeginWith(final byte[] src, final byte[] dst) {
    ...
    private int findEndIndex(final byte[] src, final byte[] dst) {
        int countEnd = src.length - dst.length;
        for (int i = 0; i <= countEnd; i++) {
            if (checkInclude(src, dst, i)) {
                return i + dst.length;
            }
        }
        return -1;
    ...
    private boolean checkInclude(final byte[] src, final  byte[] dst, int i) {
